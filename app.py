...
# ==========================================
def cek_login():
    # Inisialisasi variabel state keamanan
    ...
    if not st.session_state["logged_in"]:
        ...
        # Cek apakah sedang dalam masa hukuman (Lockout)
        if st.session_state["lockout_until"]:
            if datetime.now() < st.session_state["lockout_until"]:
                ...
                # Masa hukuman selesai, reset counter
                st.session_state["failed_attempts"] = 0
                st.session_state["lockout_until"] = None
            ...
            with st.form("form_login"):
                ...
                if submit:
                    if email == st.secrets["ADMIN_EMAIL"] and password == st.secrets["ADMIN_PASSWORD"]:
                        ...
                        st.session_state["failed_attempts"] = 0 # Reset percobaan
                        st.rerun()
                    else:
                        time.sleep(2) # Tarpitting: Perlambat bot hacker 2 detik
...
def proses_perubahan_global(key, df_sumber, nama_mode):
    ...
    # C. HAPUS DATA (Ditahan ke Modal Konfirmasi)
            ...
            if list_provinsi:
                ...
                for yayasan in list_yayasan_di_prov:
                    ...
                    if df_yayasan_filtered.empty:
                        continue # Sembunyikan yayasan jika tidak ada status yang cocok
